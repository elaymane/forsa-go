...
export default async function EditOpportunityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const citySuggestions = Array.from(new Set(allOpportunities.flatMap((o) => parseCities(o.location)))).sort();
  const levelSuggestions = Array.from(
    new Set(allOpportunities.flatMap((o) => (o.level ?? "").split(",").map((l) => l.trim()).filter(Boolean)))
  ).sort();
  const specializationSuggestions = Array.from(
    new Set(allOpportunities.flatMap((o) => (o.specialization ?? "").split(",").map((s) => s.trim()).filter(Boolean)))
  ).sort();
