...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // Real individual cities only — a multi-city opportunity like
  // "Berkane / Rabat / Marrakech" contributes 3 separate real cities here,
  // never one broken combined-string entry.
  const cities = Array.from(new Set(opportunities.flatMap((o) => parseCities(o.location))));
  ...
  const comboEntries = typeSlugs.flatMap(([slug, cfg]) =>
    cities
      .filter((city) => opportunities.some((o) => o.type === cfg.type && parseCities(o.location).includes(city)))
      .map((city) => ({
  ...
  return [
    ...
    ...organizations.map((org) => ({
      ...
      lastModified: new Date(org.lastUpdatedAt),
