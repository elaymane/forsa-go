...
export default async function DashboardPage() {
  ...
  return (
    <AppShell notifications={notifications} user={user} isAdmin={isAdminEmail(user.email)} locale={locale}>
      ...
      <DashboardHero opportunities={opportunities} applicationsMap={applicationsMap} userName={actingUser.name} />
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <OpportunitySpotlight opportunities={opportunities} applicationsMap={applicationsMap} />
        </div>
        <UpcomingTimeline events={timeline} />
      </div>

      <UnknownExamDatesSection opportunities={opportunities} applicationsMap={applicationsMap} />

      <NewOpportunitiesCarousel opportunities={opportunities} applicationsMap={applicationsMap} />
    </AppShell>
  );
}
