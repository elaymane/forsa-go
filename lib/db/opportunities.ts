...
import type { EducationLevel, Opportunity, OpportunityType, OpportunityProfile } from "@/types/opportunity";
...
interface OpportunityRow {
  ...
  profiles: unknown; // Postgres JSONB — comes back already parsed, or null
}
...
export function placeholderLogo(organization: string): string {
  ...
  const firstWord = words[0]?.replace(/[^a-zA-Z0-9À-ÿ]/g, "") ?? "";
  // If the org already starts with a real acronym (e.g. "OFPPT (Office de...)"),
  // use that directly — far more recognizable than one letter from each of the
  // first two unrelated words.
  const isAcronym = /^[A-Z]{2,}$/.test(firstWord);
  const initials = isAcronym
    ? firstWord.slice(0, 2)
    : words
        .map((w) => w.replace(/[^a-zA-Z0-9À-ÿ]/g, "")[0]) // strip leading punctuation like "(" before taking the initial
        .filter(Boolean)
        .slice(0, 2)
        .join("")
        .toUpperCase();
...
function rowToOpportunity(row: OpportunityRow, orgLogos: Record<string, string> = {}): Opportunity {
  ...
  let deadline = row.deadline ?? "Date limite non précisée";
  let daysLeft = row.days_left ?? 0;
  let date = row.date ?? "Date limite non précisée";
  ...
  if (deadlineDate) {
    ...
    deadline = daysLeft > 0 ? `Encore ${daysLeft} jour${daysLeft === 1 ? "" : "s"}` : daysLeft === 0 ? "Dernier jour" : "Fermé";
  }
  ...
  return {
    ...
    contractType:
      row.contract_type === "CDI" || row.contract_type === "CDD" || row.contract_type === "Fonction publique"
        ? row.contract_type
        : null,
    profiles: Array.isArray(row.profiles) ? (row.profiles as OpportunityProfile[]) : null,
  };
}
...
async function slugify(title: string, organization: string): Promise<string> {
  ...
  // Clean slug by default — a random suffix on every single URL is genuinely
  // worse for SEO (looks untrustworthy, wastes keyword-relevant length) than
  // it needs to be. Only fall back to a suffix if this exact slug is
  // already taken, which the app's own duplicate-prevention already makes rare.
  const existing = (await sql`SELECT id FROM opportunities WHERE id = ${base}`) as Array<{ id: string }>;
...
export async function createOpportunity(input: NewOpportunityInput): Promise<{ id: string; duplicate: boolean }> {
  ...
  const id = await slugify(input.title, input.organization);
  ...
  await sql`
    INSERT INTO opportunities
      (id, title, organization, location, type, status, deadline, days_left, date,
       ...
       specialization, grade, positions_count, website, keywords, is_public, profiles, contract_type)
    VALUES
      (${id}, ${input.title}, ${input.organization}, ${input.location}, ${input.type},
       'open', 'Date limite non précisée', 0, 'Date limite non précisée',
       ...
       ${input.profiles && input.profiles.length > 0 ? JSON.stringify(input.profiles) : null}, ${input.contractType})
  `;
...
export async function updateOpportunity(id: string, input: NewOpportunityInput): Promise<void> {
  ...
  await sql`
    UPDATE opportunities SET
      ...
      profiles = ${input.profiles && input.profiles.length > 0 ? JSON.stringify(input.profiles) : null},
