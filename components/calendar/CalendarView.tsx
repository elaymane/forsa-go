...
export default function CalendarView({ events, opportunities, applicationsMap }: CalendarViewProps) {
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
      {/* CALENDAR GRID */}
      <div className="rounded-3xl border border-black/10 bg-gradient-to-br from-white/80 to-purple-50/40 p-6 shadow-sm backdrop-blur-xl dark:border-white/10 dark:from-white/5 dark:to-purple-500/[0.03] lg:col-span-3">
      ...
      <div className="flex flex-col rounded-3xl border border-black/10 bg-white/60 p-5 shadow-sm backdrop-blur-xl dark:border-white/10 dark:bg-white/5 lg:col-span-2 lg:max-h-[640px]">
        <div className="mb-4 flex items-center justify-between">
          ...
          <span className="flex items-center gap-1 rounded-full bg-purple-500/10 px-2.5 py-1 text-xs font-semibold text-purple-600 dark:text-purple-300">
            <Sparkles size={11} /> {monthEventCount} event{monthEventCount === 1 ? "" : "s"}
          </span>
        </div>
        ...
        {monthEventGroups.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-2 text-center">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">Nothing scheduled this month.</p>
          </div>
        ) : (
              ...
              return (
                <div key={group.iso} ref={(el) => { dateRefs.current[group.iso] = el; }}>
                  ...
                  >
                    ...
                    {isToday && <span className="rounded-full bg-purple-500/10 px-2 py-0.5">Today</span>}
                  </div>
