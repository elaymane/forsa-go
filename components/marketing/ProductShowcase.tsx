  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2 rounded-lg border border-black/10 bg-black/[0.02] px-3 py-2 dark:border-white/10 dark:bg-white/5">
        ...
        <span className="text-xs text-gray-400">Search opportunities...</span>
      </div>
      {[
        { name: "ENSAM Engineering School", tag: "Concours", tone: "bg-orange-100 text-orange-700 dark:bg-orange-500/20 dark:text-orange-300" },
        { name: "OFPPT Formation Pro", tag: "Training", tone: "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300" },
      ].map((row) => (
...
function TrackMock() {
  const stages = ["Applied", "Written", "Oral", "Accepted"];
  return (
    <div className="space-y-3">
      ...
      <div className="mt-2 flex items-center gap-2 rounded-lg border border-purple-200 bg-purple-50 px-3 py-2 dark:border-purple-500/20 dark:bg-purple-500/10">
        ...
        <span className="text-xs font-medium text-purple-700 dark:text-purple-300">Written Exam stage</span>
      </div>
    </div>
  );
}
...
function NotifyMock() {
  return (
    <div className="space-y-2">
      {[
        { icon: Bell, text: "Written exam in 5 days", tone: "text-amber-500" },
        { icon: Clock, text: "Deadline in 3 days", tone: "text-red-500" },
        { icon: MapPin, text: "New opportunity from OFPPT", tone: "text-purple-500" },
      ].map((n) => {
        const Icon = n.icon;
        return (
