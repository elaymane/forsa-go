...
export default function AuthForm({ variant, title, subtitle, submitLabel, action, footer, locale }: AuthFormProps) {
  const i = t(locale).authForm;
  ...
  const FIELD_SETS = {
    login: [
      { name: "email", type: "email", placeholder: i.emailPlaceholder, icon: Mail, autoComplete: "email" },
      { name: "password", type: "password", placeholder: i.passwordPlaceholder, icon: Lock, autoComplete: "current-password" },
    ],
    signup: [
      { name: "name", type: "text", placeholder: i.fullNamePlaceholder, icon: User, autoComplete: "name" },
      { name: "email", type: "email", placeholder: i.emailPlaceholder, icon: Mail, autoComplete: "email" },
      {
        name: "password",
        type: "password",
        placeholder: i.passwordMinPlaceholder,
        icon: Lock,
          ...
          <div>
            ...
            {!checking && managerLookup && managerLookup !== "not_found" && (
              <p className="mt-1.5 flex items-center gap-1 text-xs font-medium text-emerald-600 dark:text-emerald-400">
                <CheckCircle2 size={13} /> {i.willBeLinkedTo} {managerLookup.name}
              </p>
            )}
