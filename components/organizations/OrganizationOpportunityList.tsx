...
function OpportunityRow({ offer, applicationState, guestMode }: OpportunityRowProps) {
...
export default function OrganizationOpportunityList({
  ...
  return (
    <div className="space-y-3">
      {offers.map((offer) => (
        <OpportunityRow key={offer.id} offer={offer} applicationState={applicationsMap[offer.id]} guestMode={guestMode} />
      ))}
    </div>
  );
}
